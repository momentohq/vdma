...
use libfabric_sys::{FI_READ, FI_WRITE, fi_mr_desc, fi_mr_reg, fid_domain, fid_mr};

use crate::error::check;
use crate::extent_hooks;
...
impl LocalRegions {
    ...
    pub(crate) fn descriptor(
        ...
        if let Some(memory_region) = extent_hooks::covering(start, end, domain) {
            ...
            return Ok(unsafe { fi_mr_desc(memory_region) });
        }
        ...
        let mut memory_region: *mut fid_mr = ptr::null_mut();
        {
            ...
            let _span =
                tracing::info_span!(parent: None, "region_register", bytes = length).entered();
            check(
                unsafe {
                    fi_mr_reg(
                        self.domain,
        ...
        Ok(unsafe { fi_mr_desc(memory_region) })
    }
